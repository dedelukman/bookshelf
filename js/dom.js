...
function addBookShelf() {
    ...
    const bookYear = "Tahun: " + document.getElementById("inputBookYear").value;
...
function removeBook(taskElement) {
    if (confirm("Apakah Anda yakin akan menghapus data Buku!")) {
...
function editBook(taskElement) {
    ...
    if (button === "Edit buku") {
        ...
        taskElement.querySelector(".book_item > .action > .blue").innerText = "Simpan Perubahan";
        bookTitle.focus();
    } else {
        const bookShelf = findBook(taskElement[BOOKSHELF_ITEMID]);
        ...
        if (confirm("Apakah Anda yakin akan menyimpan perubahan data Buku!")) {
            updateDataToStorage();
        }
        ...
        taskElement.querySelector(".book_item > .action > .blue").innerText = "Edit buku";
